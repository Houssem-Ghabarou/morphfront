      ...
      <div ref={cardRef} className="flex flex-col h-full animate-fade-in">
        ...
        <div className="shrink-0 px-6 pt-6 pb-4">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-semibold text-[var(--text)]">{moduleLabel}</h2>
              ...
              {selectedRows.size > 0 && (
                <>
                  ...
                  <button onClick={exportCSV}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-[var(--text-muted)] bg-white/5 border border-[var(--border)] hover:border-zinc-600 hover:text-zinc-200 transition-colors">
          ...
          <div className="relative max-w-sm">
            ...
            <input
              ...
              className="w-full bg-[var(--bg-input)] border border-[var(--border)] rounded-lg pl-9 pr-3 py-2 text-xs text-[var(--text)] placeholder-zinc-600 focus:outline-none focus:border-violet-500/40 transition-colors"
            />
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-auto scrollbar-thin rounded-t-xl border border-[var(--border)] bg-[var(--bg-card)]">
          {isLoading ? (
            ...
            <table className="w-full text-xs border-collapse">
              <thead className="sticky top-0 z-10">
                <tr className="bg-[var(--bg-input)]">
                  <th className="w-10 px-3 py-3 border-b border-[var(--border)]">
                  ...
                  {dataColumns.map((col) => {
                    ...
                    return (
                      <th key={col.column_name} onClick={() => cycleSort(col.column_name)}
                        className="text-left px-3 py-3 text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wider border-b border-[var(--border)] whitespace-nowrap cursor-pointer hover:text-zinc-200 select-none transition-colors">
                  ...
                  <th className="w-20 px-3 py-3 border-b border-[var(--border)] text-right text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody>
                {sortedRows.length === 0 ? (
                  <tr>
                    <td colSpan={dataColumns.length + 2} className="px-3 py-16 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm text-[var(--text-muted)]">{rows.length === 0 ? `No ${singularLabel.toLowerCase()} records yet` : 'No records match your search'}</p>
                        {rows.length === 0 && (
                  ...
                  sortedRows.map((row, i) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-[var(--border-muted)] transition-colors ${isDeleting ? 'opacity-40' : ''} ${isSelected ? 'bg-violet-500/8' : 'hover:bg-white/[0.02]'}`}
                      >
                          ...
                          {isDeleting ? (
                            ...
                            <div className="flex items-center justify-end gap-0.5">
                              <button onClick={() => openModal(row)} title="Edit"
                                className="w-7 h-7 flex items-center justify-center rounded-md text-[var(--text-muted)] hover:text-violet-400 hover:bg-violet-500/10 transition-colors">
                              ...
                              <button onClick={() => deleteRow(row)} title="Delete"
                                className="w-7 h-7 flex items-center justify-center rounded-md text-[var(--text-muted)] hover:text-red-400 hover:bg-red-500/10 transition-colors">
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* Pagination footer */}
          {pages > 1 && (
            <div className="flex items-center justify-between px-4 py-2.5 rounded-b-xl border border-t-0 border-[var(--border)] bg-[var(--bg-sidebar)]">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => goToPage(1)} disabled={page === 1}
                  className="w-7 h-7 flex items-center justify-center rounded-md text-[var(--text-muted)] hover:text-zinc-200 hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="First page">
                ...
                <button onClick={() => goToPage(page - 1)} disabled={page === 1}
                  className="w-7 h-7 flex items-center justify-center rounded-md text-[var(--text-muted)] hover:text-zinc-200 hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Previous page">
                ...
                <div className="flex items-center gap-0.5 mx-1">
                  {Array.from({ length: Math.min(pages, 7) }, (_, i) => {
                    ...
                    if (p < 0) return <span key={`ellipsis-${i}`} className="w-6 text-center text-[11px] text-[var(--text-subtle)]">…</span>;
                    return (
                ...
                <button onClick={() => goToPage(page + 1)} disabled={page === pages}
                  className="w-7 h-7 flex items-center justify-center rounded-md text-[var(--text-muted)] hover:text-zinc-200 hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Next page">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg>
                </button>
                <button onClick={() => goToPage(pages)} disabled={page === pages}
                  className="w-7 h-7 flex items-center justify-center rounded-md text-[var(--text-muted)] hover:text-zinc-200 hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Last page">
